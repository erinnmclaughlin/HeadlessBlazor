...
The dialog gets `role="dialog"`, `aria-modal="true"`, closes on <kbd>Escape</kbd> when
`ModalOptions.CloseOnEscape` is enabled (the default), and traps keyboard focus via
`HBFocusTrapBehavior` while it's mounted (restoring focus to whatever was focused before
the modal opened, once it closes). `ModalOptions.OverlayAttributes`/`ContentAttributes`
let you apply `class`/`style`/etc. to the overlay and dialog elements, since there's no
markup call site to put them on directly.
